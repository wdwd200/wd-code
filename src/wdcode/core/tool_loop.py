...
def run_tool_loop(client, conversation, tool_registry, max_rounds=MAX_TOOL_ROUNDS):
    ...
    for _ in range(max_rounds):
        ...
        content, calls_executed = handle_assistant_message(conversation, assistant_message, tool_registry)
...
def handle_assistant_message(conversation, assistant_message, tool_registry):
...
def execute_tool_call(tool_registry, tool_call):
    ...
    try:
        return tool_registry.execute(get_tool_name(tool_call), arguments)
